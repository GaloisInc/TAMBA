...
# This is the python equivalent of Haskell's replicateM
def all_bit_sequences (n):
    return ["".join(seq) for seq in itertools.product("01", repeat=n)]

def main ():
    if len(sys.argv) != 6:
        print("Not enough arguments, please read top of python script :)")
        exit(1)
    ...
    num_perms  = len(query_data)
    bit_seq    = all_bit_sequences(num_perms)

    for i in range(2 ** num_perms):
        out = open(file_name_prefix + str(i) + ".prob", 'w+')
        out.write(secret)
        out.write(belief)
        out.write("\n" + mpc_aid)
